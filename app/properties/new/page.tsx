...
export default function NewPropertyPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const slug = form.title.trim().replace(/[^a-z0-9]+/gi, "-").toLowerCase() || "item";
      const subfolder = `props/${slug}`;

      let mainPath: string | null = null;
      if (mainFile) mainPath = await uploadViaBackend(mainFile, subfolder);
  ...
  if (!user) {
    return (
      <PageWrapper>
        <h1 className="text-2xl font-bold mb-2">Ny annons</h1>
        <p className="mb-3">Du måste vara inloggad för att skapa en annons.</p>
        <div className="flex gap-2">
          <Link href={`/login?next=${encodeURIComponent(next)}`} className="rounded bg-black px-3 py-1 text-white">Logga in</Link>
          <Link href={`/register?next=${encodeURIComponent(next)}`} className="rounded border px-3 py-1">Skapa konto</Link>
        </div>
      </PageWrapper>
    );
  }
  ...
  if (user.role !== "host") {
    return (
      <PageWrapper>
        <h1 className="text-2xl font-bold mb-2">Ny annons</h1>
        <p>Endast värdar kan skapa annonser.</p>
        <Link href="/properties" className="underline mt-2 inline-block">Tillbaka till listan</Link>
      </PageWrapper>
    );
  }
  ...
  return (
    <PageWrapper>
      <h1 className="text-2xl font-bold mb-4">Ny annons</h1>
      ...
      <form onSubmit={onSubmit} className="space-y-4 max-w-md">
        ...
        <div>
          ...
          <input type="number" className="w-full border rounded p-2" value={form.price_per_night} onChange={(e) => setForm((f) => ({ ...f, price_per_night: Number(e.target.value) }))} min={0} />
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-gray-600 mt-1">Du kan välja flera bilder samtidigt.</p>
        </div>
